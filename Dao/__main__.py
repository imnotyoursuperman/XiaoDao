...
from Dao.modules import ALL_MODULES
from Dao.modules.helper_funcs.chat_status import is_user_admin
from Dao.modules.helper_funcs.misc import paginate_modules
                ...
                [
                    [
                        InlineKeyboardButton(
                            "ᴇxᴛʀᴀ", callback_data="more_ai_handler"
                        ),
                    ],
                    [
...
async def more_ai_handler_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "more_ai_handler":
        await query.answer()
        await query.message.edit_text(
            "*Here are the Stable Diffusion commands*:\n\n"
            "Command: /meinamix\n"
            "  • Desc: Generates an image using the meinamix checkpoint.\n\n"
            "Command: /darksushi\n"
            "  • Desc: Generates an image using the darksushi checkpoint.\n\n"
            "Command: /meinahentai\n"
            "  • Desc: Generates an image using the meinahentai checkpoint.\n\n"
            "Command: /darksushimix\n"
            "  • Desc: Generates an image using the darksushimix checkpoint.\n\n"
            "Command: /anylora\n"
            "  • Desc: Generates an image using the anylora checkpoint.\n\n"
            "Command: /cetsumix\n"
            "  • Desc: Generates an image using the cetus-mix checkpoint.\n\n"
            "Command: /darkv2\n"
            "  • Desc: Generates an image using the darkv2 checkpoint.\n\n"
            "Command: /creative\n"
            "  • Desc: Generates an image using the creative checkpoint.",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup(
                [
                    [
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("Dao.modules." + module_name)
...
async def error_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Log the error and send a telegram message to notify the jade emperor."""
...
async def gitsource_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "git_source":
        source_link = "https://github.com/imnotyoursuperman/LittleDao"
        message_text = (
...
async def Miko_about_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "Miko_":
        ...
        message_text = (
            f" <b>A.I Modules.</b>"
            f"\n <b>Standard Management and Extra Modules.</b>"
            f"\n <b>Otaku Modules.</b>"
        ...
        message_text = (
            "*Xiao Dao is the culmination of the persistent effort of a no good creator who simply wants to support his friends.*
            f"\n\n*If you find any bug in {BOT_NAME} Please report it at the support chat tagging the creator or owner of group.*"
        )
        await query.message.edit_text(
            text=message_text,
            parse_mode=ParseMode.MARKDOWN,
...
async def get_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.type != chat.PRIVATE:
        if len(args) >= 2 and any(args[1].lower() == x for x in HELPABLE):
            ...
            return
        await update.effective_message.reply_text(
            "Where do you want me to open up my menu?.",
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
...
def main():
    ...
    LOGGER.info("Xiao Dao is starting >> Using long polling.")
